...
学習内容・学習時間・を管理でき、プロジェクトとタスク単位で学習内容を記録し、ポモドロータイマーによって勉強時間を蓄積できる設計としています。また、記録した内容は振り返りや改善に活用でき、コメント機能を通じて生徒同士や講師との対話も可能です。

| | |
|--|--|
| **想定ユーザー** | 塾の生徒 |
| **解決すること** | 「勉強が続かない」「各分野にどれだけ取り組めていたか自信がない」|
| **権限制御** | 進捗や学習時間は本人のみが確認できる設計。生徒が他の生徒に見られたくないといった要望から追加 |

---

## ② 背景・課題

塾講師として働く中で、「勉強を継続することがきつい」「各分野にどれだけ取り組めていたか自信がない」と悩む生徒を多く見てきました。そこで生徒と話し合ったところ、「科目とそれに対する分野で学習・時間管理を行いたい」や「ポモドロータイマーを用いた勉強ができる環境が欲しい」という意見をもらい開発することに決めました。

生徒が何度も使いたいと思わせるように、自分が使いたい技術ではなく、生徒が使いやすいか、後から改善しやすいか、早く届けられるかの3つを軸に技術選定に取り組みました。
...
| プロジェクト作成 / 編集 / 削除 | 科目や学習テーマをプロジェクトとして管理。 |
| タスク追加 / 一斉削除 / 削除 | プロジェクト内にTODOを作成。完了済みタスクを個別または一斉に削除可能 |
...
| 画面 | できること | スクリーンショット |
|------|-----------|-----------------|
| **ホーム** | 全プロジェクトを一覧表示。未ログインでも閲覧可能 | ![ホーム画面](docs/screenshots/home.png) |
| **プロジェクト詳細 - タスク・学習時間** | タスク一覧・学習時間のサマリーを表示 | ![プロジェクト詳細](docs/screenshots/project-summary.png) |
| **プロジェクト詳細 - コメント** | コメントの投稿・閲覧 | ![コメント](docs/screenshots/project-comments.png) |
| **タイマー** | プロジェクト・タスクを選択してポモドーロタイマーを起動。学習時間が自動で記録される | ![タイマー](docs/screenshots/timer.png) |
| **プロジェクト作成・編集** | タイトル・説明・画像を設定してプロジェクトを作成・編集 | ![作成・編集](docs/screenshots/create-edit.png) |
| **プロフィール** | 自分のプロジェクト一覧を表示 | ![プロフィール](docs/screenshots/profile.png) |
| **テーマ切り替え** | UIのカラーテーマを変更 | ![テーマ切り替え](docs/screenshots/theme-selector.png) |
| **テーマ変更後** | テーマ変更後のホーム画面 | ![テーマ変更後](docs/screenshots/home-themed.png) |
...
- **Clerk** — 整ったログインUIで認証のストレスをなくし、コア機能の開発に集中
...
この経験から、機能を実装する前にデータの関係やUIなどの要件定義の重要性を身をもって実感しました。
